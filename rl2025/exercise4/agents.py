...
class DDPG(Agent):
    ...
    def __init__(
        ...
        STATE_SIZE = observation_space.shape[0] # 288
        ACTION_SIZE = action_space.shape[0] # 1
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        you should be implementing exploration using the self.noise variable that you should have declared in the __init__.
        ...
        action = self.actor(obs_tensor).squeeze(0).detach()

        if explore:  # follow an exploratory policy using noise as specified above
            noise_sample = self.noise.sample().detach()
            action += noise_sample
        # Hint: Remember to clip the action between the upper and lower bound of the action space
        # before returning the action
        return torch.clip(action, self.lower_action_bound, self.upper_action_bound).detach().cpu().numpy()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        # policy loss
        p_loss = -self.critic(torch.cat((states, policy_actions), dim=1)).mean()
