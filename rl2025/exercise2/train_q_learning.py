...
if __name__ == "__main__":
    seeds = [31, 5, 81, 39, 86, 73, 34, 93, 51, 42]
    results = {}
    for idx, config in enumerate(CONFIGS, 1):
        config.update(CONSTANTS)
        for is_slippery in [True]:
            for seed in seeds:
                path = f"QLearning_results_slippery={is_slippery}"
                ...
                evaluation_return_means.sort()
                best_mean_return = evaluation_return_means[-1]
                with open(path, "a") as f:
                    f.write(f"CONFIG: {idx}, Seed {seed}: Best Mean Return: {best_mean_return} \n")
