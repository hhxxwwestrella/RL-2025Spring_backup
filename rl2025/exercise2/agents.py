    ...
    **ONLY CHANGE THE BODY OF THE act() FUNCTION**
    ...
    def act(self, obs: int) -> int:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        """
        ### PUT YOUR CODE HERE ###
        action_values = [self.q_table[(obs, act)] for act in range(self.n_acts)]
        max_value = max(action_values)
        max_actions = [idx for idx, action_value in enumerate(action_values) if action_value == max_value]

        ### RETURN AN ACTION HERE ###
        random_number = random.random()  # a float between 0 and 1
        if random_number > self.epsilon:  # greedy action
            # randomly choose an action from max_actions
            return random.choice(max_actions)
        else:  # exploration
            # randomly choose one action from all possible actions
...
class QLearningAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        """
        ### PUT YOUR CODE HERE ###
        next_action_values = [self.q_table[(n_obs, a)] for a in range(self.n_acts)]

        if not done:
            self.q_table[(obs, action)] += self.alpha * (reward + self.gamma * max(next_action_values) - self.q_table[(obs, action)])
        else:
...
class MonteCarloAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        ### PUT YOUR CODE HERE ###
        return_value = 0
        for i in range(1, len(obses) + 1):
            # t from T-1 back to 0
            state_t, action_t, reward_t = obses[-i], actions[-i], rewards[-i]
            # update the total reward
            return_value = self.gamma * return_value + reward_t

            # increment the count
            self.sa_counts[(state_t, action_t)] = self.sa_counts.get((state_t, action_t), 0) + 1

            updated_values[(state_t, action_t)] = self.q_table.get((state_t, action_t), 0) + (1 / self.sa_counts[(state_t, action_t)]) * (return_value - self.q_table.get((state_t, action_t), 0))
