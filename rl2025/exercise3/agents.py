# TODO: already updated according to the update on 14 March
    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(timestep: int, max_timestep: int):
            ### PUT YOUR CODE HERE ###
            fraction = min(1.0, timestep / (max_timestep * self.exploration_fraction))
            return self.epsilon_start - fraction * (self.epsilon_start - self.epsilon_min)


        def epsilon_exponential_decay(timestep: int, max_timestep: int):
            ### PUT YOUR CODE HERE ###
            return max(self.epsilon_min, self.epsilon * self.epsilon_exponential_decay_factor ** (timestep/max_timestep))
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        max_actions = [idx for idx, q_value in enumerate(q_values) if q_value == max_value]
        # if there are more than one greedy actions, randomly select one.
        random_number = np.random.random()  # a float between 0 and 1
        if explore and random_number < self.epsilon:
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        ### PUT YOUR CODE HERE ###
        # print(batch)
        # Transition(states=tensor..., actions=tensor..., rewards=tensor..., done=tensor..., next_states=tensor...)
        states, actions, next_states, rewards, dones = batch

        current_q_values = self.critics_net(states).gather(1, actions.to(torch.int64))
        with torch.no_grad():
            max_next_q_values = self.critics_target(next_states).max(1, keepdim=True)[0]
            target_q_values = rewards + self.gamma * (1 - dones.unsqueeze(1)) * max_next_q_values
...
class DiscreteRL(Agent):
    ...
    def update(
        ...
        ### PUT YOUR CODE HERE ###
        next_action_values = [self.q_table[(next_state, a)] for a in range(self.n_acts)]
        self.q_table[(state, action)] += self.alpha * (reward + self.gamma * max(next_action_values) - self.q_table[(state, action)])
