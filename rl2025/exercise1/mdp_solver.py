...
class ValueIteration(MDPSolver):
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        """
        V = np.zeros(self.state_dim)  # Initialisation
        ### PUT YOUR CODE HERE ###
        while True:
            delta = 0
            # sum across all s'
            expected_returns_array = (self.mdp.P * (self.mdp.R + self.gamma * V[np.newaxis, np.newaxis, :])).sum(axis=2)
            V_new = expected_returns_array.max(axis=1)  # the a that maximises the expected return
            delta = max(delta, abs(sum(V_new - V)))
            V = V_new
            if delta < theta:
                break
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        ### PUT YOUR CODE HERE ###
        expected_returns = (self.mdp.P * (self.mdp.R + self.gamma * V[np.newaxis, np.newaxis, :])).sum(axis=2)
        # for each state, find the action that maximises the expected return
        best_actions = np.argmax(expected_returns, axis=1)
        # In the policy matrix, the best action for each state is set as 1, others 0
...
class PolicyIteration(MDPSolver):
    ...
    def _policy_eval(self, policy: np.ndarray, theta: float = 1e-6) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            prev_V = V.copy()
            expected_returns = (self.mdp.P * (self.mdp.R + self.gamma * V[np.newaxis, np.newaxis, :])).sum(axis=2)
            V = np.sum(policy * expected_returns, axis=1)
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            policy_stable = True
            V = self._policy_eval(policy)  # policy evaluation
            old_actions = np.argmax(policy, axis=1)
            expected_returns = (self.mdp.P * (self.mdp.R + self.gamma * V[np.newaxis, np.newaxis, :])).sum(
                axis=2)
            updated_actions = np.argmax(expected_returns, axis=1)

            # update the policy
            policy = np.zeros_like(policy)
            policy[np.arange(self.state_dim), updated_actions] = 1.0
            if not np.array_equal(old_actions, updated_actions) or iteration == 0:
                # In the first iteration, sometimes updated_actions and old_actions happen to be the same,
                # despite not converging.
                policy_stable = False
            if policy_stable:
                # if policy_stable, stop and return the optimal policy and V;
                # else, go to policy evaluation (back to the beginning of this loop)
                break
            iteration += 1
